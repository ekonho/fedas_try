...
class clientAS(Client):
        ...
        if is_selected:
            ...
            for step in range(max_local_epochs):
                for i, (x, y) in enumerate(trainloader):
                    ...
                    if torch.isnan(loss):
                        print(f"Client {self.id}: NaN loss detected, skipping batch")
                        continue
            ...
            for i, (x, y) in enumerate(self.load_train_data()):
                ...
                if torch.isnan(outputs).any():
                    print(f"Client {self.id}: NaN in outputs during FIM, skipping")
                    continue
            ...
            for i, (x, y) in enumerate(trainloader):
                ...
                outputs = self.model(x)
                if torch.isnan(outputs).any():
                    print(f"Client {self.id}: NaN in outputs during FIM (not selected), skipping")
                    continue
                # Negative log likelihood as our loss
                nll = -torch.nn.functional.log_softmax(outputs, dim=1)[range(len(y)), y].mean()
    ...
    def set_parameters(self, model, progress):
        ...
        if self.wo_local:
            ...
            alignment_optimizer = torch.optim.SGD(model.base.parameters(), lr=0.001)  # Adjust learning rate and optimizer as needed
            ...
            for _ in range(1):  # Iterate for 1 epochs; adjust as needed
                for x_batch, y_batch in trainloader:
                    ...
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(model.base.parameters(), max_norm=10.0)
